...
docker build --target build -t architectureplayground/simple-service:tests . && \\
echo "** Building tests docker image finished" && \\

echo "** Tests started" && \\
docker run -i --rm -v /var/run/docker.sock:/var/run/docker.sock architectureplayground/simple-service:tests && \\
echo "** Tests finished"
''')
            }
        }
...
docker build --target app -t architectureplayground/simple-service:latest . && \\
echo "** Building application docker image finished" && \\

echo "** Start pushing docker image in docker hub repository" && \\
docker push architectureplayground/simple-service:latest && \\
